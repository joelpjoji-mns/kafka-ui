...
describe('PartitionCounts', () => {
  ...
  it('shows total and per-partition pills sorted by partition', () => {
    ...
    expect(screen.getByTestId('partition-counts-total')).toHaveTextContent(
      'Total 8'
    );
...
describe('TimeRangeSelector', () => {
  ...
  it('renders all preset buttons and the timezone badge', () => {
    setup();
    ['Last 1h', 'Last 1 day', 'Last 7 days', 'Last 1 month'].forEach(
      (label) => {
        const slug = label.toLowerCase().replace(/\s+/g, '-');
        expect(
          screen.getByTestId(`time-range-preset-${slug}`)
        ).toBeInTheDocument();
      }
    );
  ...
  it('applies "Last 1 day" preset with a 24h window ending near now', async () => {
    ...
    await userEvent.click(
      screen.getByTestId('time-range-preset-last-1-day')
    );
  ...
  it('applies "Last 7 days" preset with a 7d window', async () => {
    const { onApply } = setup();
    await userEvent.click(
      screen.getByTestId('time-range-preset-last-7-days')
    );
    const [start, end] = onApply.mock.calls[0] as [Date, Date];
  ...
  it('applies "Last 1 month" preset with a 30d window', async () => {
    const { onApply } = setup();
    await userEvent.click(
      screen.getByTestId('time-range-preset-last-1-month')
    );
    const [start, end] = onApply.mock.calls[0] as [Date, Date];
    expect(end.getTime() - start.getTime()).toBe(30 * 24 * 60 * 60 * 1000);
  });

  it('shows "Clear range" when a range is set and calls onApply(null,null)', async () => {
    const start = new Date(Date.now() - 60 * 60 * 1000);
    const end = new Date();
    const { onApply } = setup(start, end);

    const clear = screen.getByRole('button', { name: /clear range/i });
    await userEvent.click(clear);

    expect(onApply).toHaveBeenCalledWith(null, null);
  });

  it('hides "Clear range" when no range is set', () => {
    setup(null, null);
    expect(
      screen.queryByRole('button', { name: /clear range/i })
    ).not.toBeInTheDocument();
  });
});
