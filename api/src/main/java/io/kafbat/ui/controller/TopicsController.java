...
import io.kafbat.ui.model.TopicDeveloperHealthDTO;
import io.kafbat.ui.model.TopicDeveloperInsightsDTO;
import io.kafbat.ui.model.TopicDeveloperMetricCategoryDTO;
import io.kafbat.ui.model.TopicDeveloperMetricDTO;
import io.kafbat.ui.model.TopicDeveloperMetricToneDTO;
import io.kafbat.ui.model.TopicDeveloperRecommendationDTO;
import io.kafbat.ui.model.TopicDeveloperRecommendationSeverityDTO;
...
public class TopicsController extends AbstractController implements TopicsApi, McpTool {
  ...
  private final TopicDataProfileService topicDataProfileService;
  private final TopicDeveloperInsightsService topicDeveloperInsightsService;
  ...
  public Mono<ResponseEntity<TopicDTO>> createTopic(
      String clusterName,
      @Valid Mono<TopicCreationDTO> topicCreationMono,
      ServerWebExchange exchange) {
    return topicCreationMono.flatMap(
        topicCreation -> {
          var context =
  ...
  @Override
  public Mono<ResponseEntity<TopicDTO>> recreateTopic(
    ...
    return validateAccess(context)
        .then(
            topicsService
                .recreateTopic(getCluster(clusterName), topicName)
                .map(clusterMapper::toTopic)
  ...
  public Mono<ResponseEntity<TopicDTO>> cloneTopic(
    ...
    return validateAccess(context)
        .then(
            topicsService
                .cloneTopic(getCluster(clusterName), topicName, newTopicName)
                .map(clusterMapper::toTopic)
                .map(s -> new ResponseEntity<>(s, HttpStatus.CREATED)))
        .doOnEach(sig -> audit(context, sig));
  }
  ...
  public Mono<ResponseEntity<Void>> deleteTopic(
    ...
    return validateAccess(context)
        .then(
            topicsService
                .deleteTopic(getCluster(clusterName), topicName)
  ...
  public Mono<ResponseEntity<Flux<TopicConfigDTO>>> getTopicConfigs(
    ...
    return validateAccess(context)
        .then(
            topicsService
                .getTopicConfigs(getCluster(clusterName), topicName)
                .map(
                    lst ->
                        lst.stream()
  ...
  public Mono<ResponseEntity<TopicDetailsDTO>> getTopicDetails(
    ...
    var contextBuilder =
        AccessContext.builder().cluster(clusterName).operationName("getTopicDetails");
    ...
    return validateAccess(context)
        .then(
            topicsService
                .getTopicDetails(getCluster(clusterName), topicName)
  ...
  @Override
  public Mono<ResponseEntity<TopicDeveloperInsightsDTO>> getTopicDeveloperInsights(
      String clusterName, String topicName, ServerWebExchange exchange) {
    var context = AccessContext.builder()
        .cluster(clusterName)
        .topicActions(topicName, VIEW)
        .operationName("getTopicDeveloperInsights")
        .build();

    return validateAccess(context)
        .then(topicDeveloperInsightsService.getInsights(getCluster(clusterName), topicName))
        .map(this::toTopicDeveloperInsights)
        .map(ResponseEntity::ok)
        .doOnEach(sig -> audit(context, sig));
  }

  private TopicDeveloperInsightsDTO toTopicDeveloperInsights(
      TopicDeveloperInsightsService.Insights insights) {
    return new TopicDeveloperInsightsDTO()
        .generatedAtMs(insights.generatedAtMs())
        .healthScore(insights.healthScore())
        .health(TopicDeveloperHealthDTO.valueOf(insights.health().name()))
        .metrics(insights.metrics().stream().map(metric -> new TopicDeveloperMetricDTO()
            .id(metric.id())
            .category(TopicDeveloperMetricCategoryDTO.valueOf(metric.category().name()))
            .label(metric.label())
            .value(metric.value())
            .detail(metric.detail())
            .tone(TopicDeveloperMetricToneDTO.valueOf(metric.tone().name())))
            .toList())
        .recommendations(insights.recommendations().stream().map(recommendation ->
            new TopicDeveloperRecommendationDTO()
                .id(recommendation.id())
                .severity(TopicDeveloperRecommendationSeverityDTO.valueOf(
  ...
  @Override
  public Mono<ResponseEntity<TopicsResponseDTO>> getTopics(
      String clusterName,
      @Valid Integer page,
    ...
    AccessContext context =
        AccessContext.builder().cluster(clusterName).operationName("getTopics").build();
            ...
            topics -> {
              ...
              var comparator =
                  sortOrder == null || !sortOrder.equals(SortOrderDTO.DESC)
                      ? comparatorForTopic
                      : comparatorForTopic.reversed();

              List<InternalTopic> filtered = topics.stream().sorted(comparator).toList();

              var totalPages =
                  (filtered.size() / pageSize) + (filtered.size() % pageSize == 0 ? 0 : 1);

              List<String> topicsPage =
                  filtered.stream()
              ...
              return topicsService
                  .loadTopics(getCluster(clusterName), topicsPage)
                  .map(
                      topicsToRender ->
  ...
  @Override
  public Mono<ResponseEntity<String>> getTopicsCsv(
      String clusterName,
    ...
    AccessContext context =
        AccessContext.builder().cluster(clusterName).operationName("getTopicsCsv").build();

    ClustersProperties.ClusterFtsProperties ftsProperties = clustersProperties.getFts();
    Comparator<InternalTopic> comparatorForTopic =
        getComparatorForTopic(orderBy, ftsProperties.use(fts));
  ...
  public Mono<ResponseEntity<TopicDTO>> updateTopic(
      String clusterName,
      String topicName,
      @Valid Mono<TopicUpdateDTO> topicUpdate,
      ServerWebExchange exchange) {
  ...
  public Mono<ResponseEntity<PartitionsIncreaseResponseDTO>> increaseTopicPartitions(
    ...
    var context =
        AccessContext.builder().cluster(clusterName).topicActions(topicName, VIEW, EDIT).build();
                ...
                .flatMap(
                    partitions ->
                        topicsService.increaseTopicPartitions(
                            getCluster(clusterName), topicName, partitions))
                .map(ResponseEntity::ok))
        .doOnEach(sig -> audit(context, sig));
  }

  @Override
  public Mono<ResponseEntity<Flux<KafkaAclDTO>>> listTopicAcls(
      String clusterName, String topicName, ServerWebExchange exchange) {
    var context =
        AccessContext.builder()
            .cluster(clusterName)
    ...
    return validateAccess(context)
        .then(
            Mono.just(
                ResponseEntity.ok(
                    aclsService
                        .listAcls(getCluster(clusterName), filter, null, false)
                ...
                .flatMap(
                    rfc ->
                        topicsService.changeReplicationFactor(
                            getCluster(clusterName), topicName, rfc))
                .map(ResponseEntity::ok))
        .doOnEach(sig -> audit(context, sig));
  }

  @Override
  public Mono<ResponseEntity<Void>> analyzeTopic(
      String clusterName, String topicName, ServerWebExchange exchange) {
    ...
    return validateAccess(context)
        .then(
            topicAnalysisService
                .analyze(getCluster(clusterName), topicName)
                .doOnEach(sig -> audit(context, sig))
                .thenReturn(ResponseEntity.ok().build()));
  }

  @Override
  public Mono<ResponseEntity<Void>> cancelTopicAnalysis(
      String clusterName, String topicName, ServerWebExchange exchange) {
    var context =
        AccessContext.builder()
            .cluster(clusterName)
  ...
  @Override
  public Mono<ResponseEntity<TopicAnalysisDTO>> getTopicAnalysis(
      String clusterName, String topicName, ServerWebExchange exchange) {
    ...
    return validateAccess(context)
        .thenReturn(
            topicAnalysisService
                .getTopicAnalysis(getCluster(clusterName), topicName)
                .map(ResponseEntity::ok)
  ...
  @Override
  public Mono<ResponseEntity<TopicDataProfileDTO>> getTopicDataProfile(
      String clusterName, String topicName, Integer sampleLimit, ServerWebExchange exchange) {
    var context =
        AccessContext.builder()
            .cluster(clusterName)
            .topicActions(topicName, MESSAGES_READ)
            .operationName("getTopicDataProfile")
            .operationParams(sampleLimit == null ? Map.of() : Map.of("sampleLimit", sampleLimit))
            .build();
    ...
    return validateAccess(context)
        .then(
            topicsService
                .getTopicDetails(getCluster(clusterName), topicName)
                .then(
                    topicDataProfileService.profile(
                        getCluster(clusterName), topicName, sampleLimit))
                .map(ResponseEntity::ok))
        .doOnEach(sig -> audit(context, sig));
  }

  @Override
  public Mono<ResponseEntity<Flux<TopicProducerStateDTO>>> getActiveProducerStates(
      String clusterName, String topicName, ServerWebExchange exchange) {
    var context =
        AccessContext.builder()
            .cluster(clusterName)
    ...
    Flux<TopicProducerStateDTO> states =
        topicsService
            .getActiveProducersState(getCluster(clusterName), topicName)
            .flatMapMany(
                statesMap ->
                    Flux.fromStream(
                        statesMap.entrySet().stream()
                            .flatMap(
                                e ->
                                    e.getValue().stream()
                                        .map(p -> clusterMapper.map(e.getKey().partition(), p)))
                            .sorted(ordering)));
  ...
  private Comparator<InternalTopic> getComparatorForTopic(
      TopicColumnsToSortDTO orderBy, boolean ftsEnabled) {
    ...
    return switch (orderBy) {
      ...
      case OUT_OF_SYNC_REPLICAS ->
          Comparator.comparing(t -> t.getReplicas() - t.getInSyncReplicas());
      ...
      case MESSAGES_COUNT ->
          Comparator.comparing(
              InternalTopic::getMessagesCount, Comparator.nullsFirst(Long::compareTo));
      default -> defaultComparator;
    };
  }

  @Override
  public Mono<ResponseEntity<Flux<FullConnectorInfoDTO>>> getTopicConnectors(
      String clusterName, String topicName, ServerWebExchange exchange) {
    var context =
        AccessContext.builder()
            .cluster(clusterName)
    ...
    Flux<FullConnectorInfoDTO> job =
        kafkaConnectService
            .getTopicConnectors(getCluster(clusterName), topicName)
            .filterWhen(
                dto -> accessControlService.isConnectAccessible(dto.getConnect(), clusterName));
